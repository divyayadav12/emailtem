...
function NavLink({
  ...
  onClose,
  pathname,
}: NavItem & { onClose?: () => void; pathname: string }) {
    ...
    <aside className="flex h-full w-[220px] shrink-0 flex-col border-r border-[#e2e6ef] bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-5 shrink-0">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center rounded text-[#6b7280] hover:bg-[#f3f4f6] md:hidden"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24">
              <path
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative z-10 h-full shadow-xl">{inner}</div>
        </div>
      )}
    </>
  );
